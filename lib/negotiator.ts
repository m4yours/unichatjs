...
export class Negotiator {
  ...
  private _setupListeners(
    ...
    const peerId = this.connection.unichat;
  ...
  cleanup(): void {
    logger.log("Cleaning up PeerConnection to " + this.connection.unichat);
  ...
  private async _makeOffer(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        logger.log("Set localDescription:", offer, `for:${this.connection.unichat}`);
  ...
  private async _makeAnswer(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        logger.log(`Set localDescription:`, answer, `for:${this.connection.unichat}`);
  ...
  async handleSDP(
    ...
    try {
      ...
      logger.log(`Set remoteDescription:${type} for:${this.connection.unichat}`);
      if (type === "OFFER") {
  ...
  async handleCandidate(ice: any): Promise<void> {
    ...
    try {
      ...
      logger.log(`Added ICE candidate for:${this.connection.unichat}`);
    } catch (err) {
      provider.emitError(PeerErrorType.WebRTC, err);
  ...
  private _addTracksToConnection(
    ...
    logger.log(`add tracks from stream ${stream.id} to unichat connection`);
